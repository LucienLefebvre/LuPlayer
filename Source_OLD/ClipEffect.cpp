...
ClipEffect::ClipEffect()
{
    ...
    compEditor.compEditedBroadcaster->addChangeListener(this);
}
...
void ClipEffect::changeListenerCallback(juce::ChangeBroadcaster* source)
{
    if (editedPlayer != nullptr)
    {
        ...
        else if (source == compEditor.compEditedBroadcaster.get())
        {
            editedPlayer->setCompParameters(editedPlayer->getCompParameters());
        }
