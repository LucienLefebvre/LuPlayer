...
public:
    ...
    void run()
    {
        ...
        /*
        std::string convertedFilesPath = "D:\"";
        //////////*****************Create FFMPEG command Line
        //add double slash to path
        std::string newFilePath = std::regex_replace(filePath, std::regex(R"(\\)"), R"(\\)");
        std::string newConvertedFilesPath = std::regex_replace(newFilePath, std::regex(R"(\\)"), R"(\\)");
        //give Output Directory
        std::size_t botDirPos = filePath.find_last_of("\\");
        std::string outputFileDirectory = filePath.substr(0, botDirPos);
        //give file name with extension
        ...
        std::string rawnamedoubleslash = std::regex_replace(rawname, std::regex(R"(\\)"), R"(\\)");
        //create entire command string
        std::string cmdstring = std::string("C:\\JUCE\\Projects\\ffmpeg.exe -i \"" + filePath + "\" -ar 48000 -y \"" + "D:" + rawname + ".mp3\" -progress \"D:" + rawname + ".txt\"");
        //std::string cmdstring = std::string("C:\\JUCE\\Projects\\ffmpeg.exe -i \"" + filePath + "\" -ar 48000 -y \"" + newConvertedFilesPath + rawnamedoubleslash + ".wav\" -progress \"D:" + rawnamedoubleslash + ".txt\"");
        //std::string cmdstring = std::string("C:\\JUCE\\Projects\\ffmpeg.exe -i \"" + filePath +"\" -ar 48000 -y \"C:\\JUCE\\Projects\\sonconverti.mp3\" -progress \"D:\\progress.txt\"");
        std::wstring w = (utf8_to_utf16(cmdstring));
        LPWSTR str = const_cast<LPWSTR>(w.c_str());
        DBG(str);
        ////////////Launch FFMPEG
