...
// bug space bar
// changer la vérification de lecture quand on veut charger une playlist (fader levé)
// baisser le gain sur réduction mono
//clic droit sur boutons de cue pour les supprimer
//pas de réduction mono au chargement du lecteur
//==============================================================================
...

{
    ...
    {
        std::unique_ptr<Settings> settings = std::make_unique<Settings>(deviceManager.getCurrentAudioDevice()->getOutputChannelNames().size());
    }
...
{  
        ...
        Settings* settings = new Settings(deviceManager.getCurrentAudioDevice()->getOutputChannelNames().size());
...
void MainComponent::resized()
{
   ...
   // playlistbisViewport.setBounds(800, playersStartHeightPosition, 745, (getParentHeight() - 40 - playersStartHeightPosition));
    ...
    if (Settings::audioOutputMode == 1 || Settings::audioOutputMode == 3)
    {
        ...
        DBG(cuelevelMeterHeight);
        DBG(meter.getPosition().getY());
...
void MainComponent::midiInitialize()
{
    ...
    midiInputList.onChange = [this] { setMidiInput(midiInputList.getSelectedItemIndex());
                             std::unique_ptr<Settings> settings;
                            settings.reset(new Settings(deviceManager.getCurrentAudioDevice()->getOutputChannelNames().size()));
