...
void Player::paintPlayHead(juce::Graphics& g, const juce::Rectangle<int>& thumbnailBounds)
{
    ...
    if (cuedrawPosition > waveformThumbnailXStart && cuedrawPosition < rightControlsStart)
...
void Player::sliderValueChanged(juce::Slider* slider)
{
    ...
    {
        transport.setGain(juce::Decibels::decibelsToGain(trimVolumeSlider.getValue()) * volumeSlider.getValue());
        cueTransport.setGain(juce::Decibels::decibelsToGain(trimVolumeSlider.getValue()));
...
void Player::setStop()
{
    if ((float)cueTransport.getCurrentPosition() > startTime)
    {
...
{
    DBG("settings audio output mode");
    DBG(Settings::audioOutputMode);
