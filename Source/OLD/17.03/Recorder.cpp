...

{
    ...
    micSlider.setSkewFactor(2, false);
    ...
    soundSlider.setSkewFactor(2, false);
...
void Recorder::resized()
{
    ...
    saveButton.setBounds(0, 60, 125, 25);
    ...
    timeLabel.setBounds(0, 90, 125, 40);
...
void Recorder::cropRecordedFile(juce::File& fileToCrop)
{
    ...
    reader.reset(outputformatManager.createReaderFor(fileToCrop));
    juce::AudioSampleBuffer* buffer = new juce::AudioSampleBuffer(1, reader->lengthInSamples);
    buffer->clear();
    ...
    //juce::AudioFormatWriter* writer = audioFormat.createWriterFor(*fos, 48000, 2, 24, juce::StringPairArray(), 0);
    DBG("before writer");
    fileStream.release();
    if (outputWriter->writeFromAudioSampleBuffer(*buffer, 0, buffer->getNumSamples()))
    {
        DBG("delete");
