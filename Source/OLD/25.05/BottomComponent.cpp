...
void BottomComponent::changeListenerCallback(juce::ChangeBroadcaster* source)
{
    if (source == recorderComponent.recordingBroadcaster)
    {
        ...
        {
            getTabbedButtonBar().setTabBackgroundColour(1, juce::Colours::red);
        }
        else
            getTabbedButtonBar().setTabBackgroundColour(1, getLookAndFeel().findColour(juce::ResizableWindow::backgroundColourId));
    }
}
