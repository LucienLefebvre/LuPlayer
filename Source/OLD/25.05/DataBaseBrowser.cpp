...
DataBaseBrowser::DataBaseBrowser() : thumbnailCache(5), thumbnail(521, formatManager, thumbnailCache), resampledSource(&transport, false, 2)
{
    ...
    conn.connect(connection_string);
        ...
        DBG("connected with " + conn.driver_name());
        conn.dbms_name();
        initialize();
    }
...
void DataBaseBrowser::paintCell(juce::Graphics& g, int rowNumber, int columnId, int width, int height, bool /*rowIsSelected*/)
{
    ...
    {
        g.drawText(secondsToMMSS(durations[rowNumber].getIntValue()/1000), 2, 0, width - 4, height, juce::Justification::centred, true);
    }
    else if (columnId == 3)
    {
...
void DataBaseBrowser::sqlQuery(std::string search)
{
    auto const connection_string = NANODBC_TEXT("Driver=ODBC Driver 17 for SQL Server;Server=localhost\\NETIA;Database=ABC4;Uid=SYSADM;Pwd=SYSADM;");
    conn.connect(connection_string);
    std::string a = "";
    std::string searchString = std::string("SELECT * FROM ABC4.SYSADM.T_ITEM WHERE ABC4.SYSADM.T_ITEM.STRING_2 LIKE '%" + search + "%' AND STATION_DELETE IS NULL");
    //juce::String searchString = juce::String("SELECT * FROM ABC4.SYSADM.T_ITEM WHERE ABC4.SYSADM.T_ITEM.STRING_2 LIKE '%a%' AND STATION_DELETE IS NULL");
...
{
    DBG(names[table.getSelectedRow()]);
