...

{
    ...
    myLayout.setItemLayout(0,          // for item 0
        ...
        -0.8);      // and its preferred size is 60% of the total available space
    ...
    myLayout.setItemLayout(2,          // for item 1
        ...
        200);        // and its preferred size is 50 pixels
...
void MainComponent::changeListenerCallback(juce::ChangeBroadcaster* source)
{
    ...
    else if (source == bottomComponent.audioPlaybackDemo.fileDraggedFromBrowser)
    {
        ...
        if (getMouseXYRelative().getX() < (soundPlayers[0]->myPlaylists[0]->getPosition().getX() + soundPlayers[0]->myPlaylists[0]->getWidth()))
            soundPlayers[0]->myPlaylists[0]->fileDragMove(getMouseXYRelative().getX(), getMouseXYRelative().getY() - 60 + playlistScrollPosition);
        else if (getMouseXYRelative().getX() > cartPosition)
            soundPlayers[0]->myPlaylists[1]->fileDragMove(getMouseXYRelative().getX() - cartPosition, getMouseXYRelative().getY() - 60 + cartScrollPoisiton);
        else
        {
