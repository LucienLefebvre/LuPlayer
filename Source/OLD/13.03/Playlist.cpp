...
Playlist::Playlist(int splaylistType)
{
    ...
    for (auto i = 0; i < playerNumber; ++i)
    {
        ...
        playlistMixer.addInputSource(&players.getLast()->mixer, false);
        playlistCueMixer.addInputSource(&players.getLast()->cueMixer, false);
        players[i]->fileName.addListener(this);
        players[i]->playerPositionLabel.addListener(this);
        players[i]->cueStopped.addListener(this);
        playersPositionLabels.add(new juce::Label);
        playersPositionLabels.getLast()->setText(juce::String(i + 1), juce::NotificationType::dontSendNotification);
        addAndMakeVisible(playersPositionLabels.getLast());
        playersPositionLabels.getLast()->setInterceptsMouseClicks(false, false);

        if (playlistType == 0)
        {
            playersPositionLabels.getLast()->setBounds(2, (105 * i) + 30, 40, 40);
            playersPositionLabels.getLast()->setFont(juce::Font(40.00f, juce::Font::plain).withTypefaceStyle("Regular"));
            //Add Remove Button
        ...
        {
            playersPositionLabels.getLast()->setBounds(getParentWidth() - 36, (105 * i) + 30, 40, 40);
            playersPositionLabels.getLast()->setFont(juce::Font(40.00f, juce::Font::plain).withTypefaceStyle("Regular"));
        ...
        players[i]->cueTransport.addChangeListener(this);
        players[i]->draggedPlayer.addListener(this);
    }
...
void Playlist::paint(juce::Graphics& g)
{
    ...
    else if (playlistType == 1)
    {
        ...
        {
            g.setColour(juce::Colour(40, 134, 189));
            g.fillRect(getParentWidth() - 36, (10 + (105 * i)), 32, 83);
        }
    }
...
void Playlist::handleFader1(int faderValue)
{
    ...
    if (spaceBarIsPlaying == false)
    {
        if (players[fader1Player] != nullptr)
        {
           ...
           std::function<void(void)> lambda = [this, faderValue]()
            {
                if (players[fader1Player] != nullptr)
    ...
    if (players[playerID] != nullptr)
    {
        ...
        for (auto i = 0; i < players.size(); i++)
        {
            if (i != fader1Player)
            {
                ...
                assignLeftFaderButtons[i]->setColour(juce::TextButton::ColourIds::buttonColourId, juce::Colours::orange);
        }
    }
    if (players[fader1Player] != nullptr)
        fader1Name.setValue(players[fader1Player]->soundName.getText());
    ...
    if (players[playerID] != nullptr)
    {
        ...
        for (auto i = 0; i < players.size(); i++)
        {
            if (i != fader2Player)
            {
                ...
                assignRightFaderButtons[i]->setColour(juce::TextButton::ColourIds::buttonColourId, juce::Colours::orange);
        }
