...
Playlist::Playlist(int splaylistType)
{
    ...
    for (auto i = 0; i < playerNumber; ++i)
    {
        ...
        playlistCueMixer.addInputSource(&players.getLast()->cueResampledSource, false);
...
void Playlist::paint(juce::Graphics& g)
{
    ...
    else if (playlistType == 1)
    {
        ...
        {
            g.setColour(juce::Colour(40, 134, 189));
            g.fillRect(getParentWidth() - 18, (10 + (105 * i)), 14, 83);
        }
    }
...
void Playlist::addPlayer(int playerID)
{
    ...
    //playlistCueMixer.addInputSource(&players[idAddedPlayer]->cueResampledSource, false);
    ...
    if (playlistType == 0)
    {
            if (players.size() > 0)
            {
                    ...
                    && !players[playerID]->keyIsPlaying)
                {
                    ...
                    playlistCueMixer.removeInputSource(&players[playerID]->cueResampledSource);
...
bool Playlist::keyPressed(const juce::KeyPress& key, juce::Component* originatingComponent)
{
    if (playlistType == 0)
    {
        ...
        {
            DBG(draggedPlayer.toString().getIntValue());
            if (players[draggedPlayer.toString().getIntValue()] != nullptr)
            {
                players[draggedPlayer.toString().getIntValue()]->playButtonClicked();
                bool playLaunchByCtrlSpace = true;
            }
        }
    ...
    for (auto i = 0; i < players.size(); i++)
    {
        if (players[i] != nullptr)
        {
            if (b == &players[i]->playerPositionLabel)
            {
                ...
                else if (i > std::min(fader1Player, fader2Player))
                {
