...
//==============================================================================
Player::Player(int index): openButton("Open"), playButton("Play"), stopButton("Stop"), cueButton("6s"), deleteButton("Delete"), thumbnailCache(5), 
...
void Player::paintPlayHead(juce::Graphics& g, const juce::Rectangle<int>& thumbnailBounds)
{
    ...
    if (drawPosition > waveformThumbnailXStart && drawPosition < rightControlsStart)
    g.drawLine(drawPosition, (float)thumbnailBounds.getY(), drawPosition,
        (float)thumbnailBounds.getBottom(), 2.0f);
    drawPlayHeadPosition = drawPosition;
    //Start Time
    if (startTimeSet)
    {
...
void Player::resized()
{
    ...
    startTimeButton.setBounds(rightControlsStart - 40  + 2, 82, cueButtonsSize, cueButtonsSize);
    stopTimeButton.setBounds(rightControlsStart - 40 + 22, 82, cueButtonsSize, cueButtonsSize);
...
void Player::spaceBarPlay()
{
    ...
    if (playButtonHasBeenClicked && !fader1IsPlaying && !fader2IsPlaying)
    {
        stop();
        playButtonHasBeenClicked = false;
        cueStopped = 1;
...
{
    playButtonHasBeenClicked = true;
    DBG(cueTransport.getLengthInSeconds());
    ...
    if (thumbnail.getNumChannels() != 0)
    {
        if (!fader1IsPlaying && !fader2IsPlaying && !keyIsPlaying)
        {
            ...
            if (mouseDragXPosition >= waveformThumbnailXStart && mouseDragXPosition <= waveformThumbnailXEnd)
            {
                ...
                mouseDragInSeconds = (((float)mouseDragRelativeXPosition * transport.getLengthInSeconds()) / (float)thumbnailBounds.getWidth());
...
void Player::setStart()
{
    if ((float)transport.getCurrentPosition() < stopTime)
    {
        startTime = (float)transport.getCurrentPosition();
        startTimeSet = true;
    }
...
void Player::setStop()
{
    if ((float)transport.getCurrentPosition() > startTime)
    {
    stopTime = (float)transport.getCurrentPosition();
    stopTimeSet = true;
    }
