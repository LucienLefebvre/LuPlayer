...
public:
    AudioPlaybackDemo()
    {
        setName("browser");
        setWantsKeyboardFocus(false);
        fileFolder = new juce::File(juce::File::getSpecialLocation(juce::File::userHomeDirectory));
        int flags = juce::FileBrowserComponent::openMode |
            juce::FileBrowserComponent::canSelectFiles |
            juce::FileBrowserComponent::filenameBoxIsReadOnly;
        //juce::WildcardFileFilter* filter = new juce::WildcardFileFilter(juce::String(T("*.*")), nullptr);

        // create the browser component
        fileBrowser = new juce::FileBrowserComponent(flags, *fileFolder, nullptr, NULL);
    ...
    void resized() override
    {
        thumbnail->setBounds(getParentWidth() / 2, 0, getParentWidth() / 2, getParentHeight() - 75);
        startStopButton.setBounds((getParentWidth() / 4)*3 - 100, getParentHeight() - 75 - 2, 200, 50);
        fileBrowser->setBounds(0, 0, getParentWidth() / 2, getParentHeight() - 25);
        autoPlayButton.setBounds(getParentWidth() / 2, getHeight() - 25, 150, 25);
