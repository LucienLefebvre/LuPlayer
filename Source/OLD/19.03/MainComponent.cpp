...
//finir recorder : gérer monitoring sur mode 4 sorties
//bug audio output mode
//demander si on veut supprimer l'enregistrement quand on clic de nouveau sur record
//afficher sur le recorder le temps en prenant compte les points in & out
//==============================================================================
    ...
    if (soundPlayers[0] != nullptr)
    {
        ...
        else if (Settings::audioOutputMode == 1)
        {
            ...
            if (bottomComponent.recorderComponent.isEnabled())
            {
                ...
                //bufferToFill.clearActiveBufferRegion();
                bufferToFill.buffer->copyFrom(1, 0, *newOutputBuffer, 0, 0, bufferToFill.buffer->getNumSamples());

                myCueMixer.getNextAudioBlock(*playAudioSource);
                soundPlayers[0]->cueloudnessMeter.processBlock(*outputBuffer);
                //bufferToFill.buffer->addFrom(1, 0, *outputBuffer, 0, 0, bufferToFill.buffer->getNumSamples());
                soundPlayers[0]->cuemeterSource.measureBlock(*outputBuffer);

                soundPlayers[0]->loudnessMeter.processBlock(*newOutputBuffer);
                soundPlayers[0]->meterSource.measureBlock(*newOutputBuffer);
...
void MainComponent::handleIncomingMidiMessage(juce::MidiInput* source, const juce::MidiMessage& message)
{
    ...
    if (message.getControllerNumber() == 45 && message.getControllerValue() == 127)
        ...
        const juce::MessageManagerLock mmLock;
             ...
             juce::MidiMessage m(juce::MidiMessage::noteOn(1, 45, juce::uint8(127)));
...
bool MainComponent::keyPressed(const juce::KeyPress &key, juce::Component* originatingComponent)
{
    if (key == 73 || key == 75 || key == 79 || key == 76)
     /*   || key == juce::KeyPress::F1Key || key == juce::KeyPress::F2Key || key == juce::KeyPress::F3Key || key == juce::KeyPress::F4Key
        || key == juce::KeyPress::F5Key || key == juce::KeyPress::F6Key || key == juce::KeyPress::F7Key || key == juce::KeyPress::F8Key
        || key == juce::KeyPress::F9Key || key == juce::KeyPress::F10Key || key == juce::KeyPress::F11Key || key == juce::KeyPress::F12Key)*/
    {
