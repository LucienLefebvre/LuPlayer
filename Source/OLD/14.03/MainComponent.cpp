...
//problème audiooutput channels -> sélection possible quand il n'y a pas 4 canaux
//faire carrés rouge pour afficher fader dans playlist
//==============================================================================
...
void MainComponent::getNextAudioBlock (const juce::AudioSourceChannelInfo& bufferToFill)
{
    ...
    cueloudnessMeter.processBlock(*outputBuffer);
    ...
    loudnessMeter.processBlock(*outputBuffer);
    meterSource.measureBlock(*outputBuffer);
    delete(outputBuffer);
    delete(playAudioSource);
    }
    //else if (Settings::audioOutputMode == 3 && (bufferToFill.buffer->getNumChannels() < 3))
    //{
    ...
    //    loudnessMeter.processBlock(*bufferToFill.buffer);
...
void MainComponent::paint(juce::Graphics& g)
{
    ...
    else if (Settings::audioOutputMode == 1)
    {
        ...
        cueShortTermLoudness = cueloudnessMeter.getMomentaryLoudnessForIndividualChannels()[0];
    }
    else if (Settings::audioOutputMode == 3)
    {
        shortTermLoudness = loudnessMeter.getMomentaryLoudness();
        cueShortTermLoudness = cueloudnessMeter.getMomentaryLoudness();
    }
    if (shortTermLoudness > 1)
        shortTermLoudness = 1;
    if (cueShortTermLoudness > 1)
        cueShortTermLoudness = 1;
    ...
    float cueLoudnessNormalized = loudnessMeterRange.convertTo0to1(cueShortTermLoudness);
    ...
    // DRAW LOUDNESS METER
    int loudnessMeterHeight = loudnessNormalized * (levelMeterHeight - meterNumericDisplayHeight - 15);
    if (shortTermLoudness >= -18.)
        g.setColour(juce::Colours::orange);
    else
        g.setColour(juce::Colour(40, 134, 189));
    g.fillRoundedRectangle(meter.getBounds().getTopRight().getX() + 7, getHeight() - levelMeterHeight + (levelMeterHeight - loudnessMeterHeight) - meterNumericDisplayHeight, 25, loudnessMeterHeight, 5);
    if (shortTermLoudness >= -18.)
    {
        float minusEighteenDbLoudness = 0.822333;
        int loudnessMeterHeightMinusEighteen = minusEighteenDbLoudness * (levelMeterHeight - meterNumericDisplayHeight - 15);
        g.setColour(juce::Colour(40, 134, 189));
        g.fillRoundedRectangle(meter.getBounds().getTopRight().getX() + 7, getHeight() - levelMeterHeight + (levelMeterHeight - loudnessMeterHeightMinusEighteen) - meterNumericDisplayHeight, 25, loudnessMeterHeightMinusEighteen, 5);
    }

    //// DRAW CUE LOUDNESS METER
    //if (Settings::audioOutputMode == 1 || Settings::audioOutputMode == 3)
    //{
    //    int cueloudnessMeterHeight = cueLoudnessNormalized * (cuelevelMeterHeight - meterNumericDisplayHeight - 15);
    //    if (cueShortTermLoudness >= -18.)
    //        g.setColour(juce::Colours::orange);
    //    else
    //        g.setColour(juce::Colour(40, 134, 189));
    //    g.fillRoundedRectangle(cuemeter.getBounds().getTopRight().getX() + 7, getHeight() - cuelevelMeterHeight - levelMeterHeight + (cuelevelMeterHeight - cueloudnessMeterHeight) - meterNumericDisplayHeight, 25, cueloudnessMeterHeight, 5);
    //    if (cueShortTermLoudness >= -18.)
    //    {
    //        float minusEighteenDbLoudness = 0.822333;
    //        int loudnessMeterHeightMinusEighteen = minusEighteenDbLoudness * (cuelevelMeterHeight - meterNumericDisplayHeight - 15);
    //        g.setColour(juce::Colour(40, 134, 189));
    //        g.fillRoundedRectangle(cuemeter.getBounds().getTopRight().getX() + 7, getHeight() - cuelevelMeterHeight - levelMeterHeight + (cuelevelMeterHeight - loudnessMeterHeightMinusEighteen) - meterNumericDisplayHeight, 25, loudnessMeterHeightMinusEighteen, 5);
    //    }
    //}
...
{
    //DBG(cueloudnessMeter.getMomentaryLoudness());
    repaint();
